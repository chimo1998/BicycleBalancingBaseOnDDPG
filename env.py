...
def init(model_name):
    global actor_model
    actor_model = tf.keras.models.load_model(model_name, compile=False)
    global buffer, ep_reward_list, avg_reward_list, time_list, ep_list, state_list, action_list
    buffer = Buffer(80000, 64)
    ep_reward_list = []
    avg_reward_list = []
    time_list = []
...
def predict(state):
    ...
    state, reward, done = get_state(state)
