...
class GY80(object):
    def __init__(self, bus=None):
        ...
        self.compass = HMC5883L(bus, 0x30, name="compass")
    ...
    def update(self):
        ...
        if abs(sqrt(sum(v_acc**2)) - 1) < 0.3:
            ...
            if(q_mag_acc[0] == None):
                return
            self._current_hybrid_orientation_q = tuple(alpha*a + (1-alpha)*b for a, b in
    ...
    def _quaternion_from_acc_mag(self, v_acc, v_mag):
        ...
        v_east /= sqrt((v_east ** 2).sum())
        v_north /= sqrt((v_north ** 2).sum())
        w,x,y,z = quaternion_from_rotation_matrix_rows(v_north, v_east, v_down)
        if (w==None):
            print('devided by 0')
            return self.current_orientation_quaternion_mag_acc_only()
...
class Gyro:
    def __init__(self):
        print('Gyro init')
        self.imu = GY80()
        x, y, z = self.imu.read_accel()
        g = sqrt(x*x + y*y + z*z)
        if abs(g - 1) > 0.3:
            sys.stderr.write('Gyro init failed')
            sys.exit(1)
        print('Gyro init done')
    ...
    def get(self):
        self.imu.update()
        #w, x, y, z = self.imu.current_orientation_quaternion_mag_acc_only()
        #print("Accel/Comp quaternion (%0.2f, %0.2f, %0.2f, %0.2f)" % (w, x, y, z))
        w, x, y, z = self.imu._current_hybrid_orientation_q
        #w, x, y, z = self.imu._current_gyro_only_q
